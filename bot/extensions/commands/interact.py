...
def make_interactions_select_menu(loc: Localization, uid: User) -> StringSelectMenu:
	...
	for id_, interaction in interactions.items():
		option_list.append(StringSelectOption(label=interaction['name'], value=f'{id_}_{uid}'))

	return StringSelectMenu(*option_list, placeholder=loc.l('interact.placeholder'), custom_id='interaction_selected')
