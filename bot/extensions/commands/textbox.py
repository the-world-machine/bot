...
class TextboxCommands(Extension):
		...
		if TextboxCommands._characters is None or (os.path.getmtime(path) != TextboxCommands._last_modified):
			with open(path, 'r') as f:
				...
				TextboxCommands._characters = [
				    Character(name=character['name'], faces=[Face(name=face['name'], emoji=face['emoji']) for face in character['faces']]) for character in data['characters']
				]
	...
	def make_characters_select_menu(locale: Localization, characters: list[Character] = None):
		...
		for character in characters:
			...
			options.append(StringSelectOption(label=name, emoji=PartialEmoji(id=character.faces[0].emoji), value=name))

		return StringSelectMenu(*options, placeholder="Select a character!", custom_id="textbox_select_char")
	...
	def make_faces_select_menu(locale: Localization, character_name: str, characters: list[Character] = None):
		...
		for face in character.faces:
			...
			options.append(StringSelectOption(label=face.name, value=face.emoji, emoji=emoji))
