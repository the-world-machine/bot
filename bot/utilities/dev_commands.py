...
async def execute_dev_command(message: Message):
	...
	if message.author.bot and str(
	    message.author.id) not in get_config('dev.whitelist'):
		return
	...
	match subcommand_name:
		case "bot":
			...
			match action:
				...
				case "sync_commands":
					msg = await message.reply(
					    f"[ Synchronizing commands... {emojis['icons']['loading']} ]"
					)
					await msg.client.synchronise_interactions()
					return await msg.edit(content=f"[ Synchronized ]")
				case _:
					return await message.reply(
					    "Available subcommands: `refresh` / `sync_commands`")
		case "eval":
			...
			if len(code) == 1 and reply_content and reply_content.startswith(
			    "```py\n"):
				code = reply_content
			elif len(code) > 1:
			...
			try:
				match method:
					case "exec":
						result = await redir_prints(exec, code, locals(),
						                            globals())
					case "aexec":
				...
				if str(exc_value) in ("py codeblock is required here",
				                      "no code provided"):
				...
				if method == "eval":
					...
					tb = TracebackException(type(value),
					                        value,
					                        tb,
					                        limit=None,
					                        compact=True)
			...
			async def handle_reply(runtime, result, note=""):
				...
				if state['raisure']:
					color = Colors.BAD
				return await message.reply(
				    embeds=Embed(color=color, description=desc))
			...
			try:
				...
				if "Description cannot exceed 4096 characters" in str(
				    e):    # TODO: paging
					return await handle_reply(
					    runtime, result[0:3900],
			...
			match args[1]:
				...
				case "reset":
					try:
						...
						return await message.reply(
						    '`[ Successfully reset shop ]`')
					except Exception as e:
						return await message.reply(f'`[ {e} ]`')
				case _:
					return await message.reply(
					    "Available subcommands: `view` / `reset`")
		case "db":
			try:
				match args[1]:
					case "set":
						...
						collection = await main.fetch_from_database(
						    await get_collection(collection, _id))
					...
					case "wool":
						...
						collection: main.UserData = await main.fetch_from_database(
						    await get_collection('user', _id))
		...
		case _:
			return await message.reply(
			    "Available commands: `eval` / `shop` / `db` / `bot`. See source code for usage"
			)
