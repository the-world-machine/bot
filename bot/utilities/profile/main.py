...
async def draw_profile(user: User, filename: str, alt: str = None, loc: Localization = Localization()) -> File:
	...
	if len(user_data.profile_description) > 0:
		...
		base_profile.text((210, 140), textwrap.fill(user_data.profile_description, 35), font=font, fill=(255, 255, 255), stroke_width=2, stroke_fill=Colors.BLACK.hex, align='center')
	...
	base_profile.text((648, 70), f'{fnum(user_data.wool, locale=loc.locale)} x',
	...
	base_profile.text((648, 32), f'{fnum(user_data.suns, locale=loc.locale)} x', font=font, fill=(255, 255, 255),
		anchor='rt',
		align='right',
		stroke_width=2,
		stroke_fill=Colors.BLACK.hex
	...
	base_profile.text((42, 251), loc.l("profile.view.image.unlocked.stamps", username=user.username),
		font=font,
		fill=(255, 255, 255),
		stroke_width=2,
		stroke_fill=Colors.BLACK.hex
	...
	alt = alt if alt is not None else loc.l("profile.view.image.alt_nodescription",
		username=username,
		suns=user_data.suns,
		wool=user_data.wool,
	)
	if len(user_data.profile_description) > 0:
		alt = loc.l("profile.view.image.alt_cont",
