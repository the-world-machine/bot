...
class ShopModule(Extension):
	...
	async def select_treasure_sell_callback(self, ctx: ComponentContext):
		...
		embed, components = await self.embed_manager(ctx,
		                                             'Sell_Treasures',
	...
	async def treasure_sell_action_callback(self, ctx: ComponentContext):
		...
		async def update():
			...
			if amount_to_sell == 'all':
				embed, components = await self.embed_manager(
				    ctx, 'Sell_Treasures', selected_treasure=treasure_id)
			else:
				embed, components = await self.embed_manager(
				    ctx, 'Sell_Treasures', selected_treasure=treasure_id)
		...
		if amount_to_sell == 'all':
			...
			sell_price = int(amount_of_treasure * treasure['price'] *
			                 stock_price)
		...
		result_text = loc.l('shop.traded_sell',
	...
	async def buy_treasure_callback(self, ctx: ComponentContext):
		...
		async def update(text: str):
			embed, components = await self.embed_manager(
			    ctx, 'Treasures', selected_treasure=treasure_id)
		...
		if amount_to_buy == 'All':
			...
			amount = min(self.max_buy_sell_limit,
		...
		owned_treasure[treasure_id] = owned_treasure.get(treasure_id,
		                                                 0) + amount
	...
	async def buy_bg_callback(self, ctx: ComponentContext):
		...
		if bg_id in owned_backgrounds:
			...
			await user.update(owned_backgrounds=owned_backgrounds,)
			embed.description = loc.l('shop.backgrounds.newly_owned',
			                          user_wool=loc.l('shop.user_wool',
			                                          wool=user.wool))
	...
	async def buy_nikogotchi_callback(self, ctx: ComponentContext):
		...
		await update(
		    loc.l('shop.nikogotchi.result',
		          amount=50_000,
		          capsule_name=capsule_loc))
	...
	@component_callback('capsules', 'pancakes', 'Backgrounds', 'Treasures',
	                    'go_back')
	...
	async def embed_manager(self, ctx: SlashContext, category: str, **kwargs):
		...
		stock: str = loc.l('shop.stocks',
		                   value=daily_shop.stock_value,
		                   price=daily_shop.stock_price)
		...
		go_back = Button(style=ButtonStyle.GRAY,
		                 custom_id='go_back',
		                 label=loc.l('shop.buttons.go_back'))
		...
		if category == 'main_shop' or category == 'go_back':
			...
			buttons = [
			    Button(label=loc.l('shop.nikogotchi.title'),
			           emoji=emojis['icons']["capsule"],
			           style=ButtonStyle.BLURPLE,
			           custom_id='capsules'),
			    Button(label=loc.l('shop.pancakes.title'),
			           emoji=emojis["pancakes"]['normal'],
			           style=ButtonStyle.BLURPLE,
			           custom_id='pancakes'),
			    Button(label=loc.l('shop.backgrounds.title'),
			           emoji=emojis['treasures']["card"],
			           style=ButtonStyle.BLURPLE,
			           custom_id='Backgrounds'),
			    Button(label=loc.l('shop.treasures.buy.title'),
			...
			button = Button(label=loc.l('shop.buttons.buy'),
			...
			description = loc.l('shop.nikogotchi.main',
			                    cost=cost,
			                    user_wool=user_wool)
			...
			nikogotchi_data: Nikogotchi = await Nikogotchi(ctx.author.id
			                                              ).fetch()
			json_data = asdict(nikogotchi_data)
			...
			for id_, pancake in enumerate(pancake_data['pancakes']):
				...
				pancake_id = pancake_id_to_emoji_index_please_rename_them_in_db(
				    pancake.id)
			...
			description = loc.l('shop.pancakes.main',
			                    items=pancake_text,
			                    user_wool=user_wool)
			...
			background_description = loc.l('shop.backgrounds.main',
			...
			buttons = [
			    ...
			    Button(label=loc.l('shop.buttons.buy'),
			           style=ButtonStyle.GREEN,
			           custom_id=f'buy_bg_{background}_{bg_page}'), go_back,
			    Button(label='>',
			           style=ButtonStyle.BLURPLE,
			...
			if selected_treasure is not None:
				...
				selected_treasure_loc: dict = loc.l(
				    f'items.treasures.{selected_treasure}')

				amount_selected = owned.get(selected_treasure, 0)

				buy_price_one = int(get_selected_treasure['price'] *
				                    daily_shop.stock_price)
				...
				max_items = min(self.max_buy_sell_limit,
			...
			for treasure in treasure_stock:
				...
				amount_owned = loc.l('shop.owned',
				                     amount=owned.get(treasure, 0))
				treasure_loc: dict = loc.l(f'items.treasures.{treasure}')

				treasure_list.append(
				    StringSelectOption(label=loc.l('shop.treasures.buy.option',
				                                   name=treasure_loc['name'],
			...
			buttons.append(
			    Button(label=loc.l('shop.treasures.sell.title'),
			           style=ButtonStyle.GREEN,
			...
			description = loc.l('shop.treasures.buy.message',
			                    stock_market=stock,
			...
			go_back = Button(label=loc.l('shop.buttons.go_back'),
			...
			if selected_treasure is not None:
				...
				sell_price_one = int(get_selected_treasure['price'] *
				                     daily_shop.stock_price)
				sell_price_all = int(get_selected_treasure['price'] *
				                     daily_shop.stock_price *
				                     owned[selected_treasure])
			...
			if selected_treasure is not None:
				...
				buttons = [
				    Button(label=loc.l('shop.buttons.sell'),
				           custom_id=f'treasure_sell_{treasure_id}_one',
				           style=ButtonStyle.GREEN),
				    Button(label=loc.l('shop.buttons.sell_all'),
			...
			embed = Embed(title=loc.l('shop.treasures.sell.title'),
			              description=loc.l('shop.treasures.sell.message',
			                                stock_market=stock,
			                                selected_treasure=treasure_details,
			                                user_wool=user_wool),
			              thumbnail=magpie,
