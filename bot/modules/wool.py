...
class WoolModule(Extension):
    ...
    @wool.subcommand()
    @slash_option(description='How much wool would you like to gamble?', name='amount', required=True, opt_type=OptionType.INTEGER, min_value=100)
    async def gamble(self, ctx:SlashContext, amount: int):
        """Moved to /gamble wool"""
        from modules.gamble import GambleModule
        return await GambleModule.wool(ctx, ctx, amount)