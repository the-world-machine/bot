...
class MusicModule(Extension):
    ...
    async def get_playing_embed(
        self, player_status: str, player: Player, allowed_control: bool
    ):
        ...
        progress_bar = make_progress_bar(
            player.position, track.duration, progress_bar_length, "square"
        )
        ...
        control_text = (
            "Everyone can control" if allowed_control else "Currently has control"
        )
        embed.set_footer(
    ...
    async def play(self, ctx: SlashContext, song: str):
        ...
        while player is None:
            try:
                player = await self.lavalink.connect(
                    voice_state.guild.id, voice_state.channel.id
                )
            except:
        ...
        if len(tracks) == 0:
            return await fancy_message(
                message, "[ No results found. ]", edit=True, color=Colors.BAD
            )
    ...
    def added_to_playlist_embed(
        self, ctx: SlashContext, player: Player, track: lavalink.AudioTrack
    ):
        ...
        add_to_queue_embed.set_author(
            name=f"Requested by {ctx.member.username}", icon_url=ctx.member.avatar.url
        )
    ...
    async def jump(self, ctx: SlashContext, position: int):
        ...
        if await self.on_cooldown(ctx.author):
            return await fancy_message(
                ctx, "[ You are on cooldown! ]", color=Colors.BAD, ephemeral=True
            )
        ...
        if len(player.queue) == 0:
            return await fancy_message(
                ctx, "[ Queue is empty! ]", color=Colors.BAD, ephemeral=True
            )

        if position > len(player.queue) or position < 0:
            return await fancy_message(
                ctx, "[ Invalid position! ]", color=Colors.BAD, ephemeral=True
            )
    ...
    async def remove(self, ctx: SlashContext, position: int = -1, own: bool = False):
        ...
        if len(queue) == 0:
            return await fancy_message(
                ctx, "[ The queue is empty! ]", color=Colors.WARN, ephemeral=True
            )
    ...
    @music.subcommand(
        sub_cmd_description="Remove the most recent song from the queue added by you."
    )
    ...
    async def get_lyrics(self, ctx: SlashContext):
        ...
        if player is None or player.current is None:
            return await fancy_message(
                ctx, "[ No tracks are currently playing! ]", ephemeral=True
            )
    ...
    async def on_player(self, player: Player, channel: GuildText):
        ...
        while player.current is not None and player_uid == player.fetch("uid"):
            ...
            user = await self.bot.fetch_member(
                player.current.requester, player.guild_id
            )
            ...
            message = await message.edit(
                content=niko, embed=embed, components=main_buttons
            )
        ...
        if stopped_track is None:
            ...
            embed.set_footer(
                text="Requested by " + requester.username, icon_url=requester.avatar_url
            )

        message = await message.edit(
            content="<:nikosleepy:1027492467337080872>", embed=embed, components=[]
        )
    ...
    async def buttons(self, ctx: ComponentContext):
        ...
        if not await self.can_modify(
            player.current.requester, ctx.author, ctx.guild.id
        ):
            return await fancy_message(
                ctx,
