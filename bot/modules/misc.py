...
class MiscellaneousModule(Extension):
	...
	async def random_wikipedia(self, ctx: SlashContext):
		...
		async with aiohttp.ClientSession() as session:
			async with session.get(
			    f'https://en.wikipedia.org/api/rest_v1/page/random/summary'
			) as resp:
				if resp.status == 200:
	...
	@slash_option(description='What sided dice to roll.',
	              min_value=1,
	              max_value=9999,
	              name='sides',
	              opt_type=OptionType.INTEGER,
	              required=True)
	@slash_option(description='How many to roll.',
	              min_value=1,
	              max_value=10,
	              name='amount',
	              opt_type=OptionType.INTEGER)
	async def roll(self, ctx: SlashContext, sides: int, amount: int = 1):
		...
		if amount == 1:
			...
			description = loc.l("misc.roll.rolled",
			                    text=sides,
			                    total=sum(rolls))

		embed = Embed(title=loc.l("misc.roll.rolling", sides=sides),
		              description=description,
		              color=Colors.DEFAULT)
		embed.set_thumbnail(
		    'https://cdn.discordapp.com/emojis/1026181557230256128.png?size=4096&quality=lossless'
		)
