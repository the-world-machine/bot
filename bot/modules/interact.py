...
def make_interactions_select_menu(loc: Localization,
                                  uid: User) -> StringSelectMenu:
	option_list = []
	...
	for id_, interaction in interactions.items():
		option_list.append(
		    StringSelectOption(label=interaction['name'], value=f'{id_}_{uid}'))

	return StringSelectMenu(*option_list,
...
class InteractModule(Extension):
	...
	async def start_interaction(self, ctx: SlashContext, who: User):
		...
		if ctx.author.id == who.id:
			return await fancy_message(ctx,
			                           loc.l('interact.twm_is_fed_up_with_you',
		...
		if who.id == self.bot.user.id:
			return await fancy_message(ctx,
			                           loc.l(
			                               'interact.twm_not_being_very_happy',
			                               user=ctx.author.mention),
			                           ephemeral=True,
			                           color=0XFF0000)
		"""if who.bot:
		...
		await ctx.send(content=loc.l('interact.selected', user=who.mention),
		               components=menu,
		               ephemeral=True)
	...
	async def menu_callback(self, ctx: ComponentContext):
		...
		await ctx.channel.send(
		    assign_variables(text,
		                     locale=ctx.locale,
		                     user_one=ctx.author.mention,
		                     user_two=user.mention))
