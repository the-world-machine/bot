...
class NikogotchiCommands(Extension):
	...
	def nikogotchi_buttons(self, ctx, owner_id: str):
		...
		return [
		    Button(
		        style=ButtonStyle.SUCCESS, label=loc.l('nikogotchi.components.pet'), custom_id=f'{prefix}pet{suffix}'
		    ),
		    Button(
	...
	async def check(self, ctx: SlashContext):
		...
		if nikogotchi.status > -1 and metadata:
			...
			if not metadata and nikogotchi.nid != "?":
				...
				await fancy_message(
				    ctx,
				    loc.l('nikogotchi.other.error.description', id=nikogotchi.nid),
				    color=Colors.BAD,
				    ephemeral=True,
				    components=buttons
				)
			...
			buttons = [
			    ...
			    Button(style=ButtonStyle.GRAY, label=loc.l('general.buttons.continue'), custom_id=f'continue {ctx.id}')
			]
	...
	async def send_away(self, ctx: SlashContext):
		...
		buttons: list[BaseComponent | dict] = [
		    Button(style=ButtonStyle.RED, label=loc.l('general.buttons.yes'), custom_id=f'rehome'),
		    Button(style=ButtonStyle.GRAY, label=loc.l('general.buttons.cancel'), custom_id=f'cancel')
		]

		await ctx.send(
		    embed=Embed(description=loc.l('nikogotchi.other.send_away.description', name=name), color=Colors.WARN),
		    ephemeral=True,
		    components=buttons
		)
	...
	async def on_rename_answer(self, ctx: ModalContext, name: str):
		...
		if ctx.custom_id.endswith("continue"):
			components.append(
			    Button(
			        style=ButtonStyle.GRAY,
			        label=loc.l('general.buttons.continue'),
			        custom_id=f'action_refresh_{ctx.author_id}'
			    )
			)
		await fancy_message(
		    ctx,
		    loc.l('nikogotchi.other.renaming.response', new_name=name, old_name=old_name),
		    ephemeral=True,
    ...
    async def trade(self, ctx: SlashContext, user: User):
        ...
        buttons = [
            Button(style=ButtonStyle.SUCCESS, label=loc.l('general.buttons.yes'), custom_id=f'trade {ctx.author.id} {uid}'),
            Button(style=ButtonStyle.DANGER, label=loc.l('general.buttons.no'), custom_id=f'decline {ctx.author.id} {uid}')
        ]
